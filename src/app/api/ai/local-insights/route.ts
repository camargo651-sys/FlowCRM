...
export async function POST() {
  ...
  const [dealsRes, activitiesRes, quotesRes, contactsRes] = await Promise.all([
    supabase.from('deals').select('id, title, value, status, expected_close_date, updated_at').eq('workspace_id', ws.id).eq('status', 'open').order('updated_at', { ascending: false }),
  ...
  // 2. Stagnation detection — based on time since last update (simplified)
  deals.filter(d => {
    ...
    actions.push({
      id: String(actionId++),
      priority: daysInStage > 30 ? 'high' as const : 'medium' as const,
      icon: 'alert',
      title: `"${d.title}" stagnant (${daysInStage} days)`,
      description: `No activity for ${daysInStage} days. Move it forward or close it.`,
      entity_type: 'deal',
      entity_name: d.title,
    })
  })
