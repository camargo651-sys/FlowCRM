...
export async function POST(request: NextRequest) {
  ...
  // Find workspace with facebook_messenger integration enabled
  const { data: integration } = await supabase
    .from('integrations')
    .select('workspace_id')
    .eq('key', 'facebook_messenger')
    .eq('enabled', true)
    .limit(1)
    .single()
    ...
    for (const event of entry.messaging || []) {
      ...
      if (ws && lead) {
        await supabase.from('notifications').insert({
          ...
          user_id: ws.owner_id,
          type: 'system',
          title: `New Facebook message from ${senderId}`,
          body: messageText.slice(0, 100) || 'New Facebook Messenger message',
          priority: 'high',
          action_url: '/leads',
        })
