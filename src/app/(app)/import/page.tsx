...
import { checkDuplicate, type DuplicateStrategy } from '@/lib/db/dedup'
    ...
    for (const entityType of ['companies', 'contacts', 'products', 'deals']) {
      ...
      for (const table of toImport) {
        ...
        for (let i = 0; i < table.rows.length; i++) {
          ...
          try {
            const buildRecord = (table: string, data: Record<string, any>) => sanitizeForInsert(supabase, table, data)
            const handleDup = async (table: string, record: Record<string, any>) => {
              const dup = await checkDuplicate(supabase, table, ws.id, record)
              if (dup.isDuplicate) {
                if (dupStrategy === 'skip') { skipped++; return false }
                if (dupStrategy === 'update') {
                  const { workspace_id: _, owner_id: __, ...updateData } = record
                  await supabase.from(table).update(updateData).eq('id', dup.existingId!)
                  imported++; return false
                }
              }
              return true // create_anyway or not duplicate
            }

            if (entityType === 'companies') {
              const name = g('name') || ''; if (!name) { skipped++; continue }
              const record = await buildRecord('contacts', { workspace_id: ws.id, name, type: 'company', email: g('email'), phone: g('phone'), website: g('website'), notes: [g('notes'), extra].filter(Boolean).join(' · ') || null, tags: ['imported'], owner_id: user.id })
              if (!await handleDup('contacts', record)) continue
              const { error } = await supabase.from('contacts').insert(record)
              if (!error) imported++; else skipped++
            } else if (entityType === 'contacts') {
              const name = g('name') || ''; if (!name) { skipped++; continue }
              const record = await buildRecord('contacts', { workspace_id: ws.id, name, type: 'person', email: g('email'), phone: g('phone'), job_title: g('job_title'), company_name: g('company_name'), notes: [g('notes'), extra].filter(Boolean).join(' · ') || null, tags: ['imported'], owner_id: user.id })
              if (!await handleDup('contacts', record)) continue
              const { error } = await supabase.from('contacts').insert(record)
              if (!error) imported++; else skipped++
            } else if (entityType === 'products') {
              const name = g('name') || ''; if (!name) { skipped++; continue }
              const record = await buildRecord('products', { workspace_id: ws.id, name, sku: g('sku'), description: [g('description'), extra].filter(Boolean).join(' · ') || null, unit_price: parseFloat(String(g('unit_price') || 0).replace(/[^0-9.-]/g, '')) || 0, cost_price: parseFloat(String(g('cost_price') || 0).replace(/[^0-9.-]/g, '')) || 0, stock_quantity: parseInt(String(g('stock_quantity') || 100)) || 100, brand: g('brand'), model: g('model'), min_stock: 5, status: 'active', tags: ['imported'] })
              if (!await handleDup('products', record)) continue
              const { error } = await supabase.from('products').insert(record)
              if (!error) imported++; else skipped++
            } else if (entityType === 'deals') {
              const title = g('title') || ''; if (!title) { skipped++; continue }
              const record = await buildRecord('deals', { workspace_id: ws.id, title, value: parseFloat(String(g('value') || 0).replace(/[^0-9.-]/g, '')) || null, status: 'open', order_index: 0, notes: [g('notes'), extra].filter(Boolean).join(' · ') || null, owner_id: user.id })
              if (!await handleDup('deals', record)) continue
